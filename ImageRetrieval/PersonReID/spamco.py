...
def spaco(configs,
  ...
  save_dir = os.path.join('logs', 'spaco', regularizer, 'seed_%d' % args.seed)
  ###########
  ...
  for view in range(num_view):
    ...
    pred_probs.append(
        mu.predict_prob(net, untrain_data, data_dir, configs[view], device))
    predictions = mu.predict_prob(net, data.trainval, data_dir, configs[view],
                                  device)
  ...
  for step in range(iter_steps):
    for view in range(num_view):
      ...
      if step + 1 == iter_steps:
        features += [
            mu.get_feature(net, query_gallery, data.images_dir, configs[view],
                           device)
        ]
